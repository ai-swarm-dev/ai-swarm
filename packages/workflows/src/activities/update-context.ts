...
export async function updateProjectContext(input: UpdateContextInput): Promise<UpdateContextOutput> {
    ...
    try {
        ...
        logger.info({ taskId: input.taskId }, 'Invoking Gemini for context analysis');

        let response = '';
        try {
            const { stdout } = await execAsync(
                `echo "${prompt.replace(/"/g, '\\"').replace(/\n/g, '\\n')}" | gemini`,
                { cwd: input.projectDir, timeout: 60000, maxBuffer: 5 * 1024 * 1024 }
            );
            response = stdout;
        } catch (geminiError) {
            logger.warn({ error: geminiError }, 'Gemini analysis failed, skipping context update');
            return { success: true, updatedFiles: [] };
        }
