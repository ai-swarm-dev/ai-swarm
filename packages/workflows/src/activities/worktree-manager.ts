 ...
 * Uses the project's SCM_TOKEN from the secrets table.
...
async function configureGitCredentials(projectId: string | undefined, cwd: string): Promise<void> {
    ...
    try {
        // Get SCM_TOKEN from project secrets
        const secrets = await projectService.getProjectSecrets(projectId);
        const scmToken = secrets['SCM_TOKEN'];

        if (!scmToken) {
            logger.debug({ projectId }, 'No SCM_TOKEN found for project, using default git config');
            return;
        }
        ...
        logger.info({ projectId }, 'Configured git credential helper with SCM_TOKEN');
    } catch (err) {
