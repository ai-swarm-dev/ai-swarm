...
export async function reviewVisualDeployment(input: VisualReviewInput): Promise<VisualReviewOutput> {
    ...
    try {
        ...
        // Try Claude first (supports vision via file path)
        let response: string | null = null;

        try {
            response = await invokeClaudeWithImage(prompt, input.screenshotPath);
        } catch (claudeError) {
            logger.warn({ error: claudeError }, 'Claude vision failed, trying Gemini');

            // Try Gemini with --media flag
            try {
                response = await invokeGeminiWithImage(prompt, input.screenshotPath);
            } catch (geminiError) {
                logger.warn({ error: geminiError }, 'Gemini vision also failed');
            }
        }
