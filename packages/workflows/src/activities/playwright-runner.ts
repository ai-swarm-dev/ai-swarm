...
export async function checkUrlAccessible(url: string, expectedStatus: number = 200): Promise<boolean> {
    ...
    try {
        ...
        const cmd = `docker exec ${PLAYWRIGHT_CONTAINER} node -e "${checkScript.replace(/\n/g, ' ').replace(/"/g, '\\"')}"`;
...
export async function captureScreenshotAsBase64(url: string): Promise<ScreenshotResult> {
    ...
    try {
        // Ensure directory exists and capture screenshot
        const captureScript = `
            const { chromium } = require('playwright');
            const fs = require('fs');
            ...
            (async () => {
                try {
                    ...
                    // Also get base64 for API compatibility
        ...
        const escapedScript = captureScript.replace(/\n/g, ' ').replace(/'/g, "'\\''");
        const cmd = `docker exec ${PLAYWRIGHT_CONTAINER} node -e '${escapedScript}'`;
